		...
		() => {
			mocha.describe("for bar charts", () => {
				mocha.describe("for flow meters", () => {
					mocha.it(
						"B15: 13 day bars for 15 minute reading intervals and flow units with +-inf start/end time & kW as kW",
						async () => {
							const unitData = [
								{
									// u4
							...
							const expected = await parseExpectedCsv(
								"src/server/test/web/readingsData/expected_bar_ri_15_mu_kW_gu_kW_st_-inf_et_inf_bd_13.csv"
							);
					...
					mocha.it(
						...
						async () => {
							...
							// Load the expected response data from the corresponding csv file
							const expected = await parseExpectedCsv(
								"src/server/test/web/readingsData/expected_bar_ri_15_mu_Thing36_gu_thing_st_-inf_et_inf_bd_13.csv"
							);
							// Create a request to the API for unbounded reading times and save the response
							const res = await chai
								.request(app)
					...
					mocha.it(
						...
						async () => {
							...
							const conversionData = [
								{
									// c4
									sourceName: "KW",
									destinationName: "KWst-inf unit",
									bidirectional: false,
									slope: 1,
									intercept: 0,
									note: "KW → KWst-inf unit",
								},
							];
							const meterData = [
								{
									name: "KW KWst-inf unit",
									unit: "KW",
									defaultGraphicUnit: "KWst-inf unit",
									displayable: true,
									gps: undefined,
									note: "",
									file: "test/web/readingsData/expected_bar_ri_15_mu_kW_gu_kWst-inf_et_inf_bd_76.csv",
									deleteFile: false,
									readingFrequency: "15 minutes",
									id: METER_ID,
								},
							...
							await prepareTest(unitData, conversionData, meterData);
							// Get the unit ID since the DB could use any value.
							const unitId = await getUnitId("KWst-inf unit");
							// Load the expected response data from the corresponding csv file
							const expected = await parseExpectedCsv(
								"src/server/test/web/readingsData/expected_bar_ri_15_mu_kW_gu_kWst-inf_et_inf_bd_76.csv"
							);
							// Create a request to the API for unbounded reading times and save the response
							const res = await chai
								.request(app)
